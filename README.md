...
> **Older versioned docs are retained for reference.** Files like `…Specification v0.3.0.docx` and `…Development Brief 07-10-2026 v0.3.0.docx` remain in the repo so the evolution of the spec is traceable. Always read the highest-numbered version first.
...
| `_dist_v0.3.2/` | Current deployment package (zipped for hosting) |
| `_dist_v0.3.1/` · earlier | Previous deployment packages (retained for reference) |
| `xoExam UI-UX Engineering Handoff Specification v0.3.2.docx` | Master handoff spec (current) |
| `xoExam Development Brief 09-18-2026 v0.3.2.docx` | Release-specific brief (current) |
| `xoExam WFOR Variance Analysis v0.3.0.docx` / `.md` | Reference-description gap analysis (discussion document) |
...
- **New in v0.3.2 (subjective protocol alignment):** The v0.3.1 console structure is unchanged; its subjective **protocol** is corrected against the **Westmead Eye Manual §8.2** on eight points — an **age-branched sphere protocol** (direct comparison at 60+/pseudophakic, fogging with +0.50 D / −0.25 D asymmetry below 60), a **round "O" JCC target two lines above best acuity**, separate higher/lower cross-cylinder magnitudes for axis and power, **handle-vs-axis** JCC depiction, active **sphere compensation** (0.25 D per 0.50 D cylinder), an **axis-midpoint** endpoint, a **RAM-GAP** duochrome endpoint leaving the patient slightly to the red, and **working-distance-based** near add. The **skip-ahead acknowledgment** on acuity-line navigation was also removed at Xenon's direction — the doctor moves between lines constantly during subjective refraction, so navigation is now free in both directions (the Visual Acuity v3 frontier model keeps its gate, where the chart is the scoring instrument). Axis before power confirmed unchanged. A **liquid-lens "protect the fog" requirement** was identified and logged for engineering — while fogging is in force, power changes must never pass through a less-plus state, or the fog is silently lost and the sphere endpoint drifts minus. Wavefront Refraction untouched; count at clinical fidelity unchanged (7 of 19). *(Prior release v0.3.1: Objective & Subjective Refraction added with a single-screen subjective console.)*
...
This prototype is **the design and interaction reference**, not the production codebase. The `.jsx` files are written for portability — inline styles, hooks-only, no exotic dependencies — so the port to React Native is mechanical: swap primitives (`<div>` → `<View>`, `<button>` → `<Pressable>`, etc.), translate inline styles to `StyleSheet`, swap `localStorage` for `AsyncStorage`. See the v0.3.2 handoff spec for the full RN portability table.
