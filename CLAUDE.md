...
- List/Grid view preference per page: `localStorage('xoexam_patients_view')`, `localStorage('xoexam_devices_view')`, `localStorage('xoexam_doctors_view')` (added v0.2.9)
...
| `ObjectiveSubjectiveRefractionTest.jsx` | **Objective & Subjective Refraction** (v0.3.2) — clone of `WavefrontRefractionTest.jsx` with every identifier re-prefixed `WFR_` → `OSR_`; component `ObjectiveSubjectiveRefractionTest`, test id `objective-subjective-refraction`. Runs independently of WFR; the subjective stage is rebuilt as a **single-screen console** (see v0.3.1 log). |
...
| `PatientsPage.jsx` | Patient list (List/Grid view toggle next to + Add Patient, persisted in `xoexam_patients_view`), search, add new |
| `DevicesPage.jsx` | Device list (List/Grid view toggle next to the action buttons, persisted in `xoexam_devices_view` — defaults to Grid), status, add new |
...
| `DoctorsPage.jsx` | Doctor list (stats + sort + List/Grid view toggle next to + Add Doctor, persisted in `xoexam_doctors_view` — defaults to Grid + ⋮ menu) + full-page DoctorProfile (6 tabs) |
...
> As of v0.2.9, no sidebar item carries a sub-menu — every item navigates directly to its section. (Patients and Devices previously had expandable sub-menus; removed because the destination pages already expose their own actions, e.g. + Add Patient / + Add Device in-page.)
...
5. **Preserve localStorage keys** — `xoexam_screen`, `xoexam_section`, `xoexam_tweaks`, `xoexam_patients_view`, `xoexam_devices_view`, `xoexam_doctors_view`.
...
> Source: competitor analysis (Marco OPD-Scan III aberrometer; Reichert Phoroptor VRx digital refractor; Supore Myopia Pro 800 biometer) + xoExam hardware Q&A answered by Steve. Steve confirmed Q3/Q7/Q8 (Jun 9, 2026) — the values below are baked into WFR. Reference doc: `xoExam Headset Sensor Questions.docx`.

### Confirmed hardware capabilities (Steve, Jun 2026)
- **Wavefront:** images a **4 mm pupil**; Zernike to **10th order (66 modes)** (lenslet-limited). **4 mm and 6 mm pupil both confirmed** (Steve, Q3, Jun 9 2026).
...
- **Vertex distance:** **25–30 mm** (confirmed — Steve).
- **Prism:** optics do **not** introduce prism; prism is **measured in software via the eye-tracking camera** (Neurolens-style).

### ✅ Resolved (Steve confirmed, Jun 9, 2026)
- **Wavefront Aberrometry analysis diameter:** built to **6.0 mm** with 6 mm HOA norms/thresholds. Steve confirmed 6 mm pupil capability (Q3) — the 6 mm build stands; no re-baseline needed.
...
- **Full 4 mm + 6 mm day/night refraction** — **both confirmed** (Steve, Q3); built and live in WFR.
...
### ✅ WFR COMPETITIVE ENHANCEMENTS — MERGED INTO PRODUCTION (v0.2.7)
CD approved all six (Jun 2026); all six are now merged into the single production `WavefrontRefractionTest.jsx`. Steve confirmed Q3/Q7/Q8 (Jun 9, 2026). All doctor-led-safe (measurements/simulations, never verdicts).
...
4. **Photopic vs. mesopic refraction** with a Diff row — **4 mm and 6 mm both confirmed** (Steve, Q3).
...
**Reversibility (the CD's key requirement) — preserved IN the one file:** every hardware value stays isolated in named constants at the top of `WavefrontRefractionTest.jsx` — `WFR_REEHANA_CONFIRMED` (true → no provisional hedging), `WFR_ANALYSIS_DIA` (4.0 mm, confirmed), `WFR_SIXMM_PROVISIONAL` (true → 6 mm columns shown), `WFR_ZERNIKE_MAX_ORDER`/`WFR_ZERNIKE_MODES` (10/66, confirmed), `WFR_VERTEX_MM` (25–30, confirmed), `WFR_SPHERE_RANGE`/`WFR_CYL_RANGE` (−14/+14 D · 0/−5 D, confirmed Q8 spec sheet). Steve confirmed Q3/Q7/Q8 (Jun 9, 2026); constants stay isolated so any later revision is a one-line edit — **no teardown, no rebuild, no separate file.** Built to the 4 mm confirmed analysis baseline throughout.

**What's now in production WFR:** (1) PSF + simulated-VA before/after (Simulation tab, habitual-vs-new); (2) binocular balance step (subjective `subjStep:'binocular'` after both eyes — fogging/alternate occlusion, no prism); (3) multi-source Rx comparison (Rx-comparison tab — objective/subjective/habitual/unaided + SE deltas, manual habitual entry); (4) photopic-vs-mesopic with night-shift Diff (Day-&-night tab — 4 mm + 6 mm column, both confirmed); (5) Smart-Cylinder auto-bracketing on the JCC (step ladder auto-sized to |cyl|, narrows one rung per reversal, toggle); (6) refraction progression tracker (Progression tab — SE trend SVG + D/yr rate vs age-banded reference, at-risk call left to clinician). Report restructured into tabs (Summary · Rx comparison · Day & night · Vision simulation · Wavefront[obj-only] · Progression). Doctor-led preserved — no Patient Classification banner, reference data only. **RESOLVED (Jun 9, 2026):** Steve confirmed Q3/Q7/Q8 — 6 mm pupil, vertex 25–30 mm, sphere −14/+14 D, cylinder 0/−5 D baked in; `WFR_REEHANA_CONFIRMED = true` removes all provisional hedging. Constants stay isolated for any later revision.
...
> **The product version is unified — the whole prototype is v0.3.2.** The "✅ vX.Y.Z" in each row's Status records *when that component last reached clinical fidelity* (the release it was last meaningfully updated in), not a separate version number for the test. A row showing an earlier release is correct and unchanged since then, not behind.
...
| v0.2.8 | Jun 9, 2026 | **Hardware answers confirmed + toggle-switch fix + full doc-set reissue** — Steve returned the Q3/Q7/Q8 headset-sensor answers and CD directed they be treated as confirmed:4 mm **and** 6 mm pupil, Zernike 10th order / 66 modes, vertex 25–30 mm, sphere-only −14/+14 D, cylinder 0/−5 D (per the opto-electrical spec sheet). `WFR_REEHANA_CONFIRMED` set **true** so no "Provisional" pills or pending captions render anywhere; the real sphere/cyl ranges replace the prior Q8 placeholders (`−20/+20 D`, `0/−10 D`); the 6 mm day/night column is now confirmed rather than provisional. **Toggle-switch fix:** the Smart-Cylinder auto-bracketing and binocular-fogging toggles were being squared by the global `button { min-height/min-width: 44px }` touch-target rule — added explicit `minWidth:38, minHeight:22` so both render as proper rectangles. **Doc set brought current together (rule 14):** CLAUDE.md, README.md, Engineering Handoff Specification v0.2.7, Development Brief 06-08-2026 v0Clinical Standards Reference v0.2.8, Clinical Evaluation Brief, and `briefs/WavefrontRefraction_Clinical_Spec_v2.md` all updated to the confirmed-hardware framing and reissued at v0.2.8; `_dist_v0.2.8/` repackaged. Version bumped v0.2.7 → **v0.2.8** (historical Jun 8 v0.2.7 entry left intact for accurate "what was touched" tracking per rule 13). |
| v0.2.9 | Jun 16, 2026 | **Shell navigation + list/grid view pass (no test or clinical changes)** — three connected shell/UX corrections at CD direction, all in `components/DashboardShell.jsx`, `components/PatientsPage.jsx`, `components/DevicesPage.jsx`, `components/DoctorsPage.jsx`. **(1) Sidebar sub-menus removed.** Patients and Devices previously expanded into sub-menus; both removed so every sidebar item now navigates directly to its section. The destination pages already expose their own primary actions (+ Add Patient / + Add Device in-page), making the sub-menu items redundant. `hasSubmenu` is now `false` for all items in `DashboardShell`. **(2) List/Grid view toggle added to Patients, Devices, and Doctors.** A segmented `List · Grid` control (consistent order and styling across all three pages — pill-group, accent active state, ≥44 px targets) lets the user switch between the table list view and a card grid. Patients defaults to List (table is the established view); Devices and Doctors default to Grid (status/identity cards are the established at-a-glance view). Each page persists its preference independently: `xoexam_patients_view`, `xoexam_devices_view`, `xoexam_doctors_view`. New views built where missing: Patients grid (patient cards: avatar, status/phase badges, diagnosis, last visit, physician), Devices list (fleet table: status, current exam, battery, uptime, next calibration), Doctors list (table: avatar, specialty, department, experience, status, patient count, ⋮ menu). All card/row clicks open the same detail/profile as before. **(3) Toggle placement standardized.** The control sits in the top action row next to the + Add button on every page, matching the Doctors layout the others were aligned to. **Doc set brought current together (rule 14):** CLAUDE.md, README.md, Engineering Handoff Specification v0.2.9, Development Brief 06-16-2026 v0.2.9, Clinical Standards Reference v0.2.9, Clinical Evaluation Brief, all reissued at v0.2.9 (clinical docs carry a shell-only/no-clinical-change note); `index.html` + `deploy.html` + `deploy-new-site.html` version strings bumped; `_dist_v0.2.9/` packaged. Count at clinical fidelity unchanged (7 of 19). |

| v0.3.0 | Jul 10, 2026 | **Test catalog visibility, ordering + filtering pass (no test or clinical changes)** — two catalog edits at CD direction, no exam component or clinical logic touched (count at clinical fidelity unchanged, 7 of 19). **(1) Tests grid reordered** in `components/TestSelection.jsx` to lead with a fixed clinical priority sequence — Wavefront Refraction → Visual Acuity → Wavefront Aberrometry → Color Vision → Visual Field → Extraocular Motility → Pupillometry — with all remaining tests following in their prior order; category filter chips still group by category. **(2) Plain "Refraction" hidden** from the Tests grid (`TestSelection.jsx`) and the patient Start-New-Test launcher (`PatientsPage.jsx`, `PATIENT_EXAM_TYPES`) — the combined Wavefront Refraction supersedes it for the refraction workflow. Component + routing retained (commented, tagged "HIDDEN (Jul 2026, CD request)"), fully restorable. Wavefront Aberrometry retained across all catalogs (Tests grid, patient launcher, Manual Control). **(3) Tests-page filtering UI hidden** — the category filter pills, the per-card uppercase category subheads, and the search box in `TestSelection.jsx` are hidden behind a `SHOW_FILTERS = false` flag (restorable); the short catalog no longer needs filtering or search. **Version numbering:** bumped v0.2.9 → **v0.3.0** (minor bump, not v0.2.10 — this is user-facing catalog behavior and reads unambiguously next to v0.2.1). **Doc set brought current together (rule 14):** CLAUDE.md, README.md, Engineering Handoff Specification v0.3.0, Development Brief 07-10-2026 v0.3.0, Clinical Standards Reference v0.3.0, Clinical Evaluation Brief, all reissued at v0.3.0 (clinical docs carry a no-clinical-change note); `index.html` + `deploy.html` version strings bumped; `_dist_v0.3.0/` packaged. |

---

## Backlog — logged decisions not yet built

### Test mode — collapsed sidebar during exams (CD direction, Sep 2026)
**Global change, applies to ALL tests. Not yet implemented — log only.**

Every test should open with the left sidebar **collapsed**, entering a strict "test mode." The sidebar opens only on deliberate action — clicking (or possibly hovering) the upper-left hamburger — and otherwise stays out of the way for the duration of the exam.

Rationale: exam screens are the densest surfaces in the product and the only ones where the doctor is working against a patient in real time. The sidebar is navigation chrome that is irrelevant once a test has started, and on a 1280×800 tablet it costs ~200–240px of width that the exam console needs. Collapsing it by default also signals a mode change — the same contextual-mode logic behind the light-admin / dark-exam split already in the design system.

Scope when built: applies on entry to any test via `ExamShell`, restores the previous sidebar state on exit, and must not break the existing `xoexam_section` persistence. Hover-to-open vs. click-only is an open UX question — click-only is safer for touch, since hover does not exist on the tablet.

| v0.3.1 | Sep 18, 2026 | **Objective & Subjective Refraction — new parallel test + single-screen subjective console.** **(1) New test component** `components/ObjectiveSubjectiveRefractionTest.jsx` — a full clone of `WavefrontRefractionTest.jsx` with all 509 identifiers re-prefixed `WFR_` → `OSR_` (plus keyframes `wfrspin/wfrpsf/wfrblur` → `osr*`), component `ObjectiveSubjectiveRefractionTest`, test id `objective-subjective-refraction`, wired into `index.html` (script tag + routing case) and placed second in the `TestSelection.jsx` catalog. **Wavefront Refraction is untouched** — the two run independently. **(2) Subjective stage rebuilt as a single-screen console** at CD direction, following the Reichert VRx / Zeiss VISUSCREEN instrument model: the former six-screen wizard (`setup → sphere → jcc-axis → jcc-power → add → binocular`) collapses to **three surfaces — scan · console · report**. Procedures became **tools** selectable in any order from a rail (MPMVA · JCC · Near add · Balance), never gated steps. **(3) The prescription matrix is the instrument** — rows SPH/CYL/AXIS/ADD × columns **Reference · Working · Δ** for both eyes; the active tool *focuses* the relevant row rather than duplicating its steppers, which removed the redundant per-tool control cards. Reference source is switchable (Objective / Habitual / Unaided), read-only, populated upstream by MPR; axis Δ uses shortest signed rotation over 180°. **(4) Chart not mirrored on the controller** — in refraction the chart is a *stimulus* (the patient judges) rather than a *scoring instrument* (as in Visual Acuity, where the v3 frontier model correctly keeps the full chart). The console shows only a compact presentation readout (20/xx + logMAR, target, mode) plus a **Peek** toggle. **(5) Duochrome added as a target**, not a procedure — red/green with the ≈20/30 validity floor surfaced inline on the level readout (noting the effect is chromatic, so color deficiency does not invalidate it). **(6) Tumbling E / Landolt C forced to single-character presentation** — line mode is locked out, since a row invites scanning and neighbour referencing in a forced-choice orientation task. Optotype "Letters" renamed **Snellen** with an "A" glyph to be visually distinct from Tumbling E. **(7) JCC unified** into one tool with an Axis | Power segmented toggle (axis first, the clinical standard — a power judgment through a misaligned cylinder is unreliable). **(8) Removed:** Smart-Cylinder auto-bracketing; the patient-response button block (the Rx controls are the input system); and the procedural guidance strip (textbook MPMVA coaching — training material, not instrument copy) — replaced by **device-state chips only** (fog applied, scan count). **(9) Fogging is adjustable** in 0.25 D steps (0–3.00 D, default +0.75) at setup, and balance fogging likewise (0.25–3.00 D, default +0.75) — both were previously hard-coded. **(10) Session setup is dismissible** (X / click-outside / Done) returning to the active tool instead of restarting the exam. **Layout verified by measurement**, not by eye: content 563px against 682px available at 1280×800. **Docs:** full rule-14 set reissued at v0.3.1; `xoExam WFOR Variance Analysis v0.3.0` (reference-description gap analysis) carried forward unchanged. |

| v0.3.2 | Sep 18, 2026 | **Objective & Subjective Refraction — clinical-protocol alignment to the Westmead Eye Manual §8.2.** Console structure from v0.3.1 unchanged; the subjective *protocol* corrected on eight points, all inside `components/ObjectiveSubjectiveRefractionTest.jsx`. **(1) Age-branched sphere protocol** — new `patientAge` + `pseudophakic` state with `OSR_noAccom`: patients **≥60 or pseudophakic** get a direct ±0.25 D plus/minus preference comparison (accommodation is not a factor, so fogging is wasted steps); patients **under 60** get the fogging protocol with the manual's asymmetric steps — **add +0.50 D**, reduce **−0.25 D**. Age and pseudophakic flag set in session setup; age expected from the patient record via MPR. **(2) JCC fixation target corrected** to a **round "O" two lines above best acuity** (was a full letter line one step larger) — a round optotype shows the astigmatic smear without letter-recognition confounds; target + level labeled, and Peek renders it. **(3) Separate JCC powers** — higher cross-cylinder for axis (`jccPowerAxis`, 0.50 default), lower for power (`jccPowerFine`, 0.25 default); previously one shared value. **(4) Handle-vs-axis alignment now depicted** — axis mode draws the JCC **handle** along the cylinder axis with the cross at ±45°, power mode aligns one JCC **axis** to it; dial labels which is in force. **(5) Sphere compensation active** — `OSR_adjustCyl` applies 0.25 D opposite sphere per 0.50 D cylinder to hold the circle of least confusion, accumulating across 0.25 D cylinder steps (`cylCompAccum`) with a pending indicator; toggleable (`autoSphComp`) for clinician override. **(6) Axis midpoint affordance** — mark bracket axes (`axisBracket`), apply computed midpoint, per the manual's "narrow range → take the middle" endpoint. **(7) Duochrome endpoint corrected** — **RAM-GAP** (red add minus, green add plus) surfaced, endpoint stated as leaving the patient **slightly on the red side** rather than strict equality. **(8) Near add follows working distance** — habitual distance selector (25/33/40/50 cm) with dioptric demand alongside the age-expected value, since add power follows daily activities and viewing distance, not age alone. **Confirmed unchanged:** axis before power. **Bug fix:** JCC Power tab header read "MPMVA". **⚠ New hardware requirement logged for MPR** in the handoff spec — liquid-lens transitions must "protect the fog": while fogging is in force a commanded power change must never pass through a state with less plus than both endpoints (monotonic move, or hold the greater plus through the transition). If the lens zeroes before setting a new value, accommodation is released, the fog is lost and the sphere endpoint drifts minus — **silently**, with nothing on screen to indicate it. No UI workaround exists; to be verified on hardware with Steve. **(9) Skip-ahead acknowledgment removed** (CD/Xenon direction) — during subjective refraction the doctor jumps acuity lines constantly, so `OSR_goLine` now navigates freely in both directions with the frontier simply following; the confirmation modal, the amber override chip on the legacy chart header, the `OSR_doOverride` recorder, `chartOverrides` state and the report's "Ahead-of-progress overrides" audit row were all deleted rather than left dormant (the session-audit panel now appears only when recorded-answer edits exist). Scope is this console only — the Visual Acuity v3 frontier model keeps its gate, where the chart is the scoring instrument and a skipped line means genuinely unscored letters. **Docs:** full rule-14 set reissued at v0.3.2. |
