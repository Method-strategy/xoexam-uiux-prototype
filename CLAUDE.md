...
> **The product version is unified — the whole prototype is v0.3.0.** The "✅ vX.Y.Z" in each row's Status records *when that component last reached clinical fidelity* (the release it was last meaningfully updated in), not a separate version number for the test. A row showing an earlier release is correct and unchanged since then, not behind.
